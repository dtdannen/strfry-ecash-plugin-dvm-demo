...
Proof of concept and performance test of using ecash for rate-limiting relay communications to DVMs on Nostr, using a Strfry relay, CDK Cashu mint.

# Components
Each of these is it's own docker container:

1. CDK Mint that issues ecash and offers microsat denominations (1 micro sat = 1 millionth of a sat). 
2. Strfry relay w/ a plugin that checks ecash is valid with our mint, and consumes it with the mint (basically spending it).
